...
*** The set of natural numbers $\color{white}\N$
    ...
    Also instead of $n^+$ we write $1$ and $0^{++}=1^+=2$ and so on,
...
*** Definition of $\color{white}\leq$
   We can define the relation $R\subset \N\time\N$ as follows:
...
*** The set $\color{white}\Z$
   ...
   - Let $=_\Z\subset \N\times\N$ and $(m,n)=_\Z(m',n')\iff m+n'=m'+n$.
...
*** Embedding of $\color{white}\N$ into $\color{white}\Z$
    - There is a homomorphism from $\N$ to the equivalence classes
      which contain the pairs $(m,)$ ($m\in\N$)
...
*** Grupoid, Semigroup, Monoid, Group, Abelian group
    - A set $G$ and an operation $\otimes:G\to G$ is a *grupoid* if for
      each $g,g'\in G$ the result of $g\otimes g'\in G$ (\ie the
      operation does not "lead out" of the set).
    - A $(G,+)$ groupoid is a *semigroup* if the operation is
      associative.
    - A $(G,+)$ semigroup is a *monoid* if there is an element $n\in
      G$ such that $n$ is the neutral element \ie $g\otimes n = n
      \otimes g = g$ for each $g\in G$.
    - A $(G,+,n)$ monoid is a *group* if every element has an inverse
...
*** Definition of $\color{white}\Q$
    ...
    - The $=\Q$ equivalence relation defines a classification of
...
*** Operations in polar form
    Let $z,w\in \C$, $n\in\N$ $z=r(\cos t+i\sin t)$ and $q=s(\cos q+i\sin q)$
    - multiplication: $zw=rs(\cos(t+q)+i\sin(t+q))$
    - division: $z/w=rs(\cos(t-q)+i\sin(t-q))$
    - exponentiation: $z^n=r(\cos(nt)+i\sin(nt))$
