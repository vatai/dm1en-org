...
*** Reminder: Solving $x^2+px+q=0$
    ...
    the square roots.  By substituting $y=x-b/3$ into \eqref{eq:cubcd}
    #+BEGIN_EXPORT latex
...
*** The set of complex numbers                                 :B_definition:
    ...
    *rectangular form* of $z$.  $\Re(z)=a$ is the *real part* and
    $\Im(z)=b$ is the *imaginary part* of $z$.  *Important warning*:
    $\Im(z)\neq bi$!
...
*** Statement                                                     :B_theorem:
    ...
    :BEAMER_env: theorem
    :END:
    Let $|z|=|a+bi|\ge 0$, then $|z|=|a+bi|=0 \iff z=a+bi=0$
...
*** Reciprocal, multiplicative inverse :B_definition:
    ...
    $x\cdot y = 1$.  For $r\in\R\setminus\{0\}$ the reciprocal if
...
** Representation of complex numbers
   ...
   % cos varphi
   \draw[color=red] (0,0) -- (1/2,0)   (1/4,-0.1) node{$\cos \varphi$};

   % sin varphi
   \draw[color=blue] (1/2,0) -- \zcords (0.6,0.4) node[rotate=90]{$\sin \varphi$};
...
*** Polar form :B_definition:
    ...
    :BEAMER_env: definition
    :END:
    The *polar form* of the non-zero $z\in \mathbb{C}$ is
    *$z=r(\cos \varphi + i \sin \varphi)$*,
    where $r>0$ is the *absolute value* (or length) of $z$.
*** Remarks
    ...
    - The polar form is not unique:
      ...
      \end{equation*}
      #+END_EXPORT
    - $z=a+bi$ is the rectangular form;
    - $z=r(\cos \varphi +i \sin \varphi)$ is the polar form.
    Here $a=r\cos \varphi, b=r \sin \varphi$.
...
*** Using the inverse of $\tan$
    ...
    [\frac{-\pi}{2},0]$ is possible.  But this can be fixed by adding $2\pi$.
...
*** Equality in polar form :B_theorem:
    ...
    :BEAMER_env: theorem
    :END:
    Two complex number $z=|z|(\cos \varphi +i \sin \varphi )$ and
    $w=|w|(\cos \psi+i \sin \psi)$ in polar form are *equal* if
    #+BEGIN_EXPORT latex
    \[
    ...
    if $|z|=|w|$ and $\varphi = \psi +k \cdot 2 \pi$ for some $k \in \mathbb{Z}$
*** Taking the \(n\)-th root: Let $z^n=w$:
    # #+BEGIN_EXPORT latex
    If $z^n=|z|^n(\cos n\varphi +i \sin n\varphi )=|w|(\cos \psi +i \sin \psi )$
    # #+END_EXPORT
    then:
    - $|z|^n=|w| \Rightarrow |z|=\sqrt[n]{|w|}$ and
    - $n\varphi=\psi+k \cdot 2\pi$ for some $k \in \mathbb{Z}$, \ie
      - $\varphi= \frac{\psi}{n}+k \cdot \frac{2\pi}{n}$
...
*** Taking the \(n\)-th root                                      :B_theorem:
    ...
    \sin\left(\frac{\varphi}{n}+\frac{2k\pi}{n}\right)\right)$$
    $k=0,1,\dots, n-1$.
...
*** Properties of order                                           :B_theorem:
    ...
    - either has all pairs of integer powers different, then
      $o(z)=\infty$,
    - of if there are two integer powers equal, and then powers of $z$
      are periodic, with the period equal to $o(z)$.
    Also, $o(z)=\min\{d:z^d=1\}$, \ie the order is the smallest
...
*** Proof                                                           :B_proof:
    ...
    $z^k=z^\ell$.  Suppose $k>l$. Then $z^{k-\ell}=1$.

    Let $d$ be the smallest positive integer, such that $z^d=1$.  If
    $z^n=1$, then if we divide $n$ by $d$ (with a remainder) \ie
    $n=q\cdot d+r$, wher $0\le r <d$. So $1=z^n=z^{q\cdot d+r}=(z^d)^q
    z^r=1^qz^r=z^r$. Since $d$ is the smallest such integer$r=0$
    implying $d\mid n$.  In the other direction: $d\mid n \Rightarrow z^n=1$.
    So: $d\mid n \iff z^n=1$.
** Primitive roots of unity
    Not all \(n\)-th roots of unity have order equal to $n$:

*** 4-th roots of unity egységgyökök: $1,i,-1,-i$.
    - $o(1)=1$;
    - $o(-1)=2$
    - $o(i)=4$
...
*** Primitive roots of unity :B_example:
    ...
    - Primitive 5-th roots of unity: $\dots$ (HF)
