...
*** Proof
    ...
    is the definition of $x\preceq y$ or $x\preceq y$.
** Intervals
...
*** Definition (Initial segment)
    The subset $\{y\in X: y<x\}$ is the *initial segment* associated
    with the element $x$.
*** Example
    Let $X$ the power set of $\{a,b,c\}$ with the /subset/ relation.
    Then the starting slice associated with $\{a,b\}$ is:
...
*** Definition(Least, greatest, minimal, maximal elements)
    ...
    - *min.*  elem. if $\forall y \, (y\in X \Rightarrow  \lnot(y\preceq x))$ 
      \ie $\lnot\exists y\, (y\in X \land y \preceq x)$;
    - *max.*  elem. if $\forall y \, (y\in X \Rightarrow  \lnot(x\preceq y))$
      \ie $\lnot\exists y\, (y\in X \land x \preceq y)$;
...
*** Definition (Injective, surjective and bijective)
    - The function $f:X\to Y$ is
      - *injective*, if $f(x)=y \land f(x')=y \Rightarrow x=x'$;
...
*** Definition
    ...
    $\oplus$ operation. The $f: X\to Y$ function is
...
*** Example
    ...
    - Let $X=\Z$ with the $+$ operation, $Y=\Z_3$ with the $+_3$
      ...
      /homomorphism/: $k+n \bmod 3 = (k \bmod 3)+_m (n \bmod 3)$.
