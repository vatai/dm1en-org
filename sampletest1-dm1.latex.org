...
4. Provided the following relation on the set $E$ of eight complex
   ...
   *Solution*:
   ...
   The image might be a little confusing, so the relation is as follows:
   ...
   Ergo $R$ is not an equivalence relation, or an ordering, so there
   is no maximal/minimal element, and no classification.
