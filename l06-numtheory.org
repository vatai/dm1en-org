...
* Euclidean division
** Euclidean division
   The primary tool of number theory is Euclidean division:
...
*** Representation of numbers					  :B_theorem:
    ...
    can uniquely be written in the form $\alert{\displaystyle
    n=\sum_{i=0}^k a_i q^i}$, where $0\leq a_i<q$ is an integer,
    $a_k\neq 0$.
...
** Numeral systems
   ...
   $n$ written in the base $q$ numeral system: $\displaystyle
...
*** Representation 						  :B_example:
   ...
   | $5$ | $3$   | $\color{red}1$ | $\frac{3-1}{2}$   | ${\color{white}1}\alert{1}10011$ |
   | $6$ | $1$   | $\color{red}1$ | $\frac{1-1}{2}$   | ${\color{white}}\alert{1}110011$ |
* Euclidean algorithm
...
*** Notation
    ...
    Let $[a,b]=\lcm(a,b)$ be the *non-negative* least common multiple!
** Greatest common divisor
...
*** Proof							    :B_proof:
    ...
    :BEAMER_env: proof
    :END:
    If on of the numbers is $0$, then the other one will be the gcd.
...
*** Dirichlet's theorem 					  :B_theorem:
    ...
    :BEAMER_env: theorem
    :END:
    If $a,d$ are integers, $d>0$, $(a,b)=1$, then there are infinitely
